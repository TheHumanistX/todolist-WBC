...
const HeaderBar = ( ) => {
    const theme = useTheme(useContext
    ...
    const { listName, tasks } = selectedTaskList || {};
    ...
    return (
        <Box display="flex" justifyContent="space-between" alignItems="center" p={2} width='100%' height='50px' padding="0px 30px" borderRadius='25px'  bgcolor='cardBackgroundColor.main' >
            ...
            >{`List: ${ selectedTaskList ? listName : ""}`}</Typography>
            <Typography variant="h6"
            style={{
                color: theme.palette.text.primary
            }}
            >Tasks Completed: {selectedTaskList ? `${tasksCompleted} of ${totalTasks}` : ""}</Typography>
            <Typography variant="h6"
            style={{
                color: theme.palette.text.primary
            }}
            >{`Date: ${currentDate}`}</Typography>
        </Box>
    );
};
