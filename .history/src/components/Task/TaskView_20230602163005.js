  ...
  return (
    <>
      ...
      <Box display="flex" justifyContent="space-around" padding={5} width='100%'>
        ...
        <Typography variant="h6" style={{ color: theme.palette.textColor.primary }}>Created: {selectedTask.createdDate}</Typography>
        {/* Display the due date if available */}
        {selectedTask.dueDate && <Typography variant="h6"  style={{ color: theme.palette.textColor.primary }}>Due: {selectedTask.dueDate}</Typography>}
      </Box>
      {/* Horizontal divider */}
      <Divider variant="middle" width="80%" />
      {/* Display the task's name in alarger font */}
      <Typography variant="h2" mb={2} mt={4} textAlign="center"  style={{ color: theme.palette.textColor.primary }}>
        {selectedTask.taskName}
      </Typography>
      {/* Display the "Information" title */}
      <Typography variant="h5" ml={2}  style={{ color: theme.palette.textColor.primary }}>
        Information
      </Typography>
