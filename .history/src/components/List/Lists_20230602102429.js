...
const Lists = ({ toggleColorMode, themeMode, theme }) => {
    ...
    return (
        ...
        >
            ...
            {taskLists.map((taskList) => {
                return <TaskList key={taskList.listId} taskList={taskList} theme={theme} />
            })}

            {/* This Card component contains an AddButton component. The button calls the setOpen function from TodoContext when clicked. */}
            <Card variant="plain" sx={{ backgroundColor: 'button.main', mt: 'auto', ml: 'auto' }}>
                <CardContent>
