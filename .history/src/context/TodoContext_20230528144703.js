...
export const TodoProvider = ({ children }) => {
    ...
    const handleTaskToggleCompleted = (listId, taskId) => {
        ...
        // If the task is now completed, remove it from the uncompleted tasks and add it to the completed tasks
        if (updatedTask.completed) {
            const updatedUncompletedTasks = uncompletedTasks.filter((task) => task.taskId !== taskId);
            const updatedCompletedTasks = [...completedTasks, updatedTask];
    
        // If the task is now uncompleted, remove it
         // Update the taskIds for the uncompleted and completed tasks
         const updatedUncompletedTasksWithIds = updatedUncompletedTasks.map((task, index) => {
            return { ...task, taskId: index + 1 };
        });

        const updatedCompletedTasksWithIds = updatedCompletedTasks.map((task, index) => {
            return { ...task, taskId: updatedUncompletedTasksWithIds.length + index + 1 };
        });

    } else {
        // If the task is now uncompleted, remove it from the completed tasks and add it to the uncompleted tasks
        const updatedCompletedTasks = completedTasks.filter((task) => task.taskId !== taskId);
    ...
    // Merge the updated uncompleted tasks and completed tasks
    const updatedTasks = [...updatedUncompletedTasksWithIds, ...updatedCompletedTasksWithIds];
