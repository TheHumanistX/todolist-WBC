...
export const TodoProvider = ({ children }) => {
    ...
    const [currentListTitle, setCurrentListTitle] = useState(null); // ! Come back and update variable name to be more inline with ListAdd
    ...
    const handleListUpdate = (updatedList) => {
        // Update the selectedTaskList with the new listName
        const updatedSelectedTaskList = { ...selectedTaskList, listName: updatedList.listName };
    
        // Get the index of the selectedTaskList in the taskLists array
        const selectedIndex = taskLists.findIndex((taskList) => taskList.listId === selectedTaskList.listId);
    
        // Replace the selectedTaskList with the updated one in the taskLists array
        const updatedTaskLists = [
          ...taskLists.slice(0, selectedIndex),
          updatedSelectedTaskList,
          ...taskLists.slice(selectedIndex + 1),
        ];
        ...
        // Set the selectedTaskList state to the updated task list
        setSelectedTaskList(updatedSelectedTaskList);
    };
